...
class TRigid extends TObject {
  ...
  unpick() {
    ...
    this.isPicked = false;
    if (this.wrapMesh) this.wrapMesh.dispose();
  }
  ...
  clone() {
    ...
    const tmp = new Constr(this.args);

    tmp.getMesh().material = this.material.clone();
    tmp.material = this.material.clone();
