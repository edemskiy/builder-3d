...
class TRoom {
  constructor(height, width, length, name, scene) {
    ...
    const floor = new TFloor({ height: length, width, depth, name: `${name}:floor`, scene });

    floor.material.diffuseColor = new BABYLON.Color3(1, 0.85, 0.62);
    this.floor = floor;

    const backWall = new TWall({ height, width: floor.width, depth, name: `${name}:backWall`, scene });
    backWall.rotateY(Math.PI);
    this.backWall = backWall;

    const frontWall = new TWall({ height, width: floor.width, depth, name: `${name}:frontWall`, scene });
    this.frontWall = frontWall;

    const rightWall = new TWall(
      { height: backWall.height, width: floor.height, depth, name: `${name}:rightWall`, scene });

    rightWall.rotateY(Math.PI / 2);
    this.rightWall = rightWall;

    const leftWall = new TWall(
      { height: backWall.height, width: floor.height, depth, name: `${name}:leftWall`, scene });
