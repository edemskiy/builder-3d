...
class ObjectsMenu extends Component {
  ...
  addMesh() {
    const name = Object.keys(this.refs).filter(item => this.refs[item].name === uploadTypes.objects && this.refs[item].checked)[0];
    if (name) {
      if (Object.keys(this.objects).some(item => item === name)) {
        this.objects[name][objectsStruct.constructor](this.objects[name][objectsStruct.defaultParams]);
      } else {
        T3DObject({ name, src: `data:${this.props.customObjects.get(name)}`, scene: this.props.scene });
      }
    }
  }
    ...
    return (
      <div className="elements-block">
        ...
        <div className="flex-row">
          <div
            className="tabs"
            onChange={() => this.props.setNewActiveControlMenu(
          ...
          >
            ...
            <div className="obj-menu-tab">
              <div className="obj-group">
                ...
                <input type="radio" name={uploadTypes.objects} ref={basicObjects.TWall} id={basicObjects.TWall} />
                <label htmlFor={basicObjects.TWall} >Стена</label>
                <input type="radio" name={uploadTypes.objects} ref={basicObjects.TFloor} id={basicObjects.TFloor} />
