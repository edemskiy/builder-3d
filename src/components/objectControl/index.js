...
class ObjectControl extends Component {
  componentDidUpdate(prevProps) {
    if (
      this.props.pickedObjects.size > 0 &&
    ...
    if (
      prevProps.pickedObjects !== this.props.pickedObjects &&
      this.props.pickedObjects.size !== 0
    ) {
      ...
      this.refs.xPosition.value = (this.groupObject
        ? this.groupObject.center.x
        : this.currentMeshObject.getPosition().x
      ).toFixed(1);
      this.refs.yPosition.value = (this.groupObject
        ? this.groupObject.center.y
        : this.currentMeshObject.getPosition().y
      ).toFixed(1);
      this.refs.zPosition.value = (this.groupObject
        ? this.groupObject.center.z
        : this.currentMeshObject.getPosition().z
      ).toFixed(1);

      this.refs.rotateY.value = (
        (this.groupObject
          ? this.groupObject.getRotationY()
          : this.currentMeshObject.getRotationY()) *
        (180 / Math.PI)
      ).toFixed(0);

      this.refs.widthInput.value = this.groupObject
        ? 0
        : this.currentMeshObject.width;
      this.refs.heightInput.value = this.groupObject
        ? 0
        : this.currentMeshObject.height;
      this.refs.depthInput.value = this.groupObject
        ? 0
        : this.currentMeshObject.depth;
      /* *************************************** */
    }
  ...
  setAxisRestrictions() {
    this.props.setNewAxisRestrictions({
      x: this.refs.axisX.checked,
      y: this.refs.axisY.checked,
      z: this.refs.axisZ.checked
    });
  }

  setAdheranceObjects() {
    this.props.setNewAdheranceObjects({
      toObjects: this.refs.adheranceToObjects.checked,
      toGrid: this.refs.adheranceToGrid.checked
    });
  }

  changeSize() {
    this.currentMeshObject.setSize(
      parseFloat(this.refs.heightInput.value),
      parseFloat(this.refs.widthInput.value),
      parseFloat(this.refs.depthInput.value)
    );
  }

  changePosition() {
    this.groupObject
      ? this.groupObject.setPosition(
          parseFloat(this.refs.xPosition.value),
          parseFloat(this.refs.yPosition.value),
          parseFloat(this.refs.zPosition.value)
        )
      : this.currentMeshObject.setPosition(
          parseFloat(this.refs.xPosition.value),
          parseFloat(this.refs.yPosition.value),
          parseFloat(this.refs.zPosition.value)
        );
  ...
  changeRotation() {
    this.groupObject
      ? this.groupObject.rotateY(
          (parseFloat(this.refs.rotateY.value) * Math.PI) / 180
        )
      : this.currentMeshObject.rotateY(
          (parseFloat(this.refs.rotateY.value) * Math.PI) / 180
        );
  }
    ...
    return (
      <div className="hidden elements-block" id="objectControls" ref="main">
        <div className="flex-row content-start">
          <button
            onClick={() =>
              setObjectInteraction(ObjInteractionEventsState.ungroup)
            }
            className="blue"
            id="unGroup"
          >
          ...
          <button
            onClick={() =>
              setObjectInteraction(ObjInteractionEventsState.clone)
            }
            className="blue"
            id="cloneObject"
          >
          ...
          <button
            onClick={() =>
              setObjectInteraction(ObjInteractionEventsState.delete)
            }
            className="blue"
            id="deleteObject"
          >
        ...
        <div
          className="changeSize sett-block flex-row"
          id="changeSize"
          ref="changeSize"
        ...
        <div className="flex-row rotateY">
          ...
          <div
            className="sett-block flex-row"
            id="changePosition"
            onChange={() => this.changePosition()}
          >
            <div>
              x:{" "}
              <input type="number" step="0.1" id="xPosition" ref="xPosition" />
            </div>
            <div>
              y:{" "}
              <input type="number" step="0.1" id="yPosition" ref="yPosition" />
            </div>
            <div>
              z:{" "}
              <input type="number" step="0.1" id="zPosition" ref="zPosition" />
            </div>
          </div>
        </div>

        <div
          className="axisSettings  sett-block flex-row"
          onChange={() => this.setAxisRestrictions()}
        >
          <div className="flex-row">
            ...
            <input
              ...
              id="axisX"
              ref="axisX"
              onChange={() => (this.refs.axisY.checked = false)}
            />
            <label htmlFor="axisX" />
          </div>
          <div className="flex-row">
            ...
            <input
              ...
              ref="axisY"
              onChange={() =>
                (this.refs.axisZ.checked = this.refs.axisX.checked = !this.refs
                  .axisY.checked)
              }
            />
            <label htmlFor="axisY" />
