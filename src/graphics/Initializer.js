...
class Initializer {
    ...
    new BABYLON.Vector3(DefaultCamera.position.x, DefaultCamera.position.y, DefaultCamera.position.z), this.scene);
    camera.setTarget(new BABYLON.Vector3(DefaultCamera.target.x, DefaultCamera.target.y, DefaultCamera.target.z))
    camera.attachControl(this.canvas);
    this.defautCamera = camera;
  }

  createDefaultGround(){
    const ground = BABYLON.MeshBuilder.CreateGround(DefaultGround.name, { height: DefaultGround.height, width: DefaultGround.width }, this.scene);
  ...
  createDefaultRoom(){
    const room1 = new TRoom(DefaultRoom.height, DefaultRoom.width, DefaultRoom.depth, DefaultRoom.name, this.scene);
    room1.setPosition(0,0,0);
  }

  createDefaultLight(){
    const light = new BABYLON.HemisphericLight(DefaultLight.name,
     new BABYLON.Vector3(DefaultLight.direction.x, DefaultLight.direction.y, DefaultLight.direction.z), this.scene);
