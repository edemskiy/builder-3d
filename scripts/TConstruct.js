class TConstruct{
	constructor(pickedWall, currentPosition, pickedPoint, addingObject, addingMode){
        let objectPosition = {
            x: Math.floor(pickedPoint.x) - currentPosition.x + pickedWall.width/2,
            y: (addingObject.name === "door") ? addingObject.height/2 : currentPosition.y - pickedWall.height/2 + Math.floor(pickedPoint.y),
