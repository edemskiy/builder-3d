...
class TWall extends TRigid {
   constructor(options) {
      ...
      const wall = BABYLON.MeshBuilder.CreateBox(name, {height: height, width: width, depth: depth, updateble: true}, this.scene);
      ...
      let multi = new BABYLON.MultiMaterial(name + "Material",this.scene);

      multi.subMaterials.push(new BABYLON.StandardMaterial(name + "BackMaterial", this.scene));
      multi.subMaterials.push(new BABYLON.StandardMaterial(name + "FrontMaterial", this.scene));
   ...
   setTexture(name) {
      this.material.subMaterials[0].diffuseTexture = new BABYLON.Texture(name, this.scene);
      this.material.subMaterials[1].diffuseTexture = new BABYLON.Texture(name, this.scene);
   }
   ...
   addObject(addingObject, xPos, yPos) {
      ...
      const newMeshWall = wallCSG.toMesh(this.name, this.material, this.scene);
   ...
   deleteObject(object) {
      ...
      const newMeshWall = solidWallCSG.toMesh(this.name, this.material, this.scene);
      newMeshWall.checkCollisions = this.collision;
      newMeshWall.getObject = () => this;
      this.meshArr[0] = newMeshWall;
